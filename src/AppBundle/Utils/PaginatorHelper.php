...
/**
 * Upravena trida paginator z nette
...
class PaginatorHelper
{
	...
	public function getPageOffer() {
		$minRange = $this->getPage() - 2;
		$maxRange = $this->getPage() + 1;
		$showPage = array();
		$showPage[] = $this->getFirstPage();
		foreach (range($minRange, $maxRange) as $page) {
			if ($page < 1 || $page == $this->getFirstPage()) {
				continue;
			}
		...
		//nasleduje doplneni mezery pred poslednim a po prvnim
		$showPage[] = $this->getLastPage();
		if ($showPage[1] != 2) { // druha hodnota musi byt 2 jinak gap
			$insert = array('gap');
			array_splice($showPage, 1, 0, $insert);
		}
		end($showPage);
		$lastKey = key($showPage);
		if ($showPage[$lastKey - 1] + 1 != $showPage[$lastKey]) { // predposledni musi byt o jedna mensi nez posledni jinak gap
			$insert = array('gap');
			array_splice($showPage, $lastKey, 0, $insert);
		}
		return $showPage;
	}          
